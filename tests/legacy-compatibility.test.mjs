...
const relative = file => path.relative(legacyRoot, file).split(path.sep).join('/');
const pages = originalFiles.filter(file => file.endsWith('.html'));
const images = originalFiles.filter(file => relative(file).startsWith('images/'));
...
test('All 9 legacy pages are exported at their original URLs', async () => {
  ...
  for (const file of pages) {
    ...
    assert.ok(sitemap.includes(`<loc>${url}</loc>`), `Missing original URL in sitemap: ${url}`);
    const html = await readFile(path.join('out', rel), 'utf8');
    assert.ok(html.includes(`rel="canonical" href="${url}"`), `Changed canonical: ${url}`);
  }
});
...
test('Original favicons and public documents retain their URLs and contents', async () => {
  for (const rel of ['favicon.ico', 'favicon2.ico', 'documents/laurea.pdf', 'documents/abilitazione.pdf']) {
    assert.equal(digest(await readFile(path.join('out', rel))), digest(await readFile(path.join(legacyRoot, rel))));
  }
});
