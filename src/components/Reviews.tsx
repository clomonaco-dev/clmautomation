    ...
    fetch("/.netlify/functions/google-reviews?lang=" + locale, {
      ...
      .then((value) => {
        if (
          ...
          !Number.isFinite(value.userRatingCount)
        )
          throw new Error();
        setData({
          ...value,
  ...
  return (
    <section className="section shell reviews">
      <div className="section-heading">
        ...
        <a
          className="text-link"
          href={safeLink(data?.googleMapsUri) || mapsUrl}
          target="_blank"
      ...
      {data && data.reviews.length > 0 ? (
        <>
          <p className="review-score">
            <strong>
              {data.rating.toLocaleString(locale, {
                minimumFractionDigits: 1,
                maximumFractionDigits: 1,
              })}
            </strong>{" "}
            / 5 · {data.userRatingCount} {t.reviewCount} <span>Google</span>
          </p>
          <div className="card-grid">
            {data.reviews.map((r, i) => (
              <blockquote className="review-card" key={r.author + i}>
                ...
                <footer>
                  <strong>
                    {safeLink(r.authorUri) ? (
                      <a
                        href={safeLink(r.authorUri)}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        {r.author}
          ...
          <p role="status">
            {status === "loading"
              ? en
                ...
                : "Caricamento delle recensioni Google…"
              : status === "error"
                ? en
                  ? "Reviews are temporarily unavailable. You can still read them on Google."
                  : "Le recensioni non sono al momento disponibili qui. Puoi leggerle sul profilo Google."
                : en
